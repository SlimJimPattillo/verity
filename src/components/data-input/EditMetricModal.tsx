...
import { Hash, DollarSign, Percent, Users, Pencil } from "lucide-react";
import { cn } from "@/lib/utils";

interface EditMetricModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSave: (id: string, metric: Omit<Metric, "id">) => void;
  metric: Metric | null;
}
...
export function EditMetricModal({ open, onOpenChange, onSave, metric }: EditMetricModalProps) {
  ...
  // Load metric data when modal opens or metric changes
  useEffect(() => {
    if (metric) {
      ...
      setUnit(metric.unit);
      setType(metric.type);
      setComparison(metric.comparison || "");
      setPreviousValue(metric.previousValue?.toString() || "");
      setShowAdvanced(!!(metric.comparison || metric.previousValue));
    }
  }, [metric]);

  const handleSave = () => {
    if (!label.trim() || !value || !metric) return;

    const updatedMetric: Omit<Metric, "id"> = {
      label: label.trim(),
    ...
    onSave(metric.id, updatedMetric);
    onOpenChange(false);
  };
